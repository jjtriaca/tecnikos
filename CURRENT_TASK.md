...
## 📌 Frente ATUAL: **FISCAL (NFS-e)** — v1.13.18→v1.13.25 NO AR. **v1.13.22 = fix do 495 (Cenario B)**. **ACHADO (v1.13.24/25)**: SLS = setup MANUAL na Focus (`focusNfeCompanyId` vazio + tokens colados) → o botao "Registrar empresa" (caminho da plataforma/token de revenda) **NAO se aplica** ao SLS (da `requisicao_invalida`). **Pro SLS o Cenario B se faz NO PAINEL da Focus**: desabilitar "Ambiente da NFSe Nacional" (manter NFSe padrao ON) — com Layout=Nacional aqui, emite via Rlz. v1.13.24=provider expoe corpo do erro; v1.13.25=botao detecta setup manual + catalogo `requisicao_invalida`. **TESTE PENDENTE**: painel Focus (ambiente nacional OFF) + retentar RPS 33. Doc: [memory/nfse-nacional-cenario-b.md].
