  ...
  autoActions: {
    ...
    proximityTrigger: {
      ...
      onEnterRadius: {
        ...
        updateAddressCoords: boolean;      // salvar coordenadas GPS no endereço ao entrar no raio
        coordsRadiusMeters: number;        // raio interno para gravar coordenadas (ex: 10m)
      };
      arrivalButton: {
...
export const TRIGGER_OPTIONS: TriggerDefinition[] = [
  ...
  { id: 'os_evaluation_created',   entity: 'SERVICE_ORDER', event: 'evaluation_created',     icon: '📋', label: 'OS de Avaliação/Orçamento',             description: 'Quando uma OS de avaliação (visita para orçar) é criada' },
...
export const AUTO_ACTION_LABELS: Record<string, { label: string; icon: string; hint: string }> = {
  ...
  notifyTecnico:  { label: 'Notificar técnico',                icon: '👷', hint: 'Dispara quando a OS entra nesta etapa. O técnico recebe a mensagem pelo canal configurado.' },
...
function createEmptyStage(status: string, label: string, icon: string): StageConfig {
  return {
    ...
    execLinkLayout: [
      ...
      { id: 'ex_sig', type: 'signature', label: 'Assinatura do cliente',   enabled: false },
...
export const WORKFLOW_PRESETS: WorkflowPreset[] = [
  ...
  },
  {
    ...
    apply: (cfg) => {
      ...
      caminho.autoActions.notifyCliente = { enabled: true, channel: 'whatsapp', message: 'O técnico está a caminho. OS: {titulo}' };
