...
export function getDefaultConfig(type: BlockType): Record<string, any> {
  switch (type) {
    ...
    case 'MATERIALS': return { label: 'Materiais e relatório do serviço', noteLabel: 'Descrição dos serviços prestados', notePlaceholder: 'Descreva os serviços realizados e observacoes...', noteRequired: false, saveNoteTo: 'serviceDescription', itemLabel: 'Descrição do material', itemsRequired: false, minItems: 1, saveItemsTo: 'materialsUsed', confirmButton: { label: 'Enviar relatório', color: 'green', icon: '📦' } };
