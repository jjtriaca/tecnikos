    ...
    <div className="flex flex-col" style={{ height: "calc(100vh - 4.5rem)" }}>
      ...
      <div className="flex flex-wrap items-center gap-x-1 gap-y-0.5 px-2 py-0.5 bg-white border-b border-slate-200 shrink-0" style={{ zoom: 0.7 }}>
        ...
        {tab === "Inicio" && (<>
          ...
          <label className="text-xs text-slate-600 flex items-center gap-1" title="Largura da página (mm)">L<NumInput spin value={pageDims(layout.branding).w} onChange={(v) => setBranding({ pageWidthMm: v || null })} className="w-14 rounded border border-slate-300 px-1 py-1 text-sm" />mm</label>
          <label className="text-xs text-slate-600 flex items-center gap-1" title="Altura da página (mm) — diminua p/ folha mais baixa">A<NumInput spin value={pageDims(layout.branding).h} onChange={(v) => setBranding({ pageHeightMm: v || null })} className="w-14 rounded border border-slate-300 px-1 py-1 text-sm" />mm</label>
        ...
        {tab === "Layout" && (selectedBox ? (() => {
          ...
          return (<>
            ...
            {isDyn ? (<>
              ...
              {sb.stack ? <label className="text-xs text-slate-600 flex items-center gap-1" title="Espaço (mm) entre os grupos empilhados na impressão">Espaço<NumInput spin key={`${sb.id}-sgap`} value={sb.stackGap ?? 2} onChange={(v) => patchSelBox({ stackGap: v })} className="w-12 rounded border border-slate-300 px-1 py-1 text-sm" />mm</label> : null}
            ...
            <label className="text-xs text-slate-600 flex items-center gap-1" title="Borda (px)">Borda<NumInput spin key={`${sb.id}-bw`} value={sbst.borderWidth ?? 0} onChange={(v) => patchSelStyle({ borderWidth: v || null })} className="w-12 rounded border border-slate-300 px-1 py-1 text-sm" /><input type="color" value={sbst.borderColor || "#e2e8f0"} onChange={(e) => patchSelStyle({ borderColor: e.target.value })} className="h-5 w-6 cursor-pointer border-0 bg-transparent p-0" /></label>
            <label className="text-xs text-slate-600 flex items-center gap-1" title="Cantos (px)">Cantos<NumInput spin key={`${sb.id}-rad`} value={sbst.radius ?? 0} onChange={(v) => patchSelStyle({ radius: v || null })} className="w-12 rounded border border-slate-300 px-1 py-1 text-sm" /></label>
            <label className="text-xs text-slate-600 flex items-center gap-1" title="Espacamento interno (px)">Padding<NumInput spin key={`${sb.id}-pad`} value={sbst.padding ?? 0} onChange={(v) => patchSelStyle({ padding: v || null })} className="w-12 rounded border border-slate-300 px-1 py-1 text-sm" /></label>
            ...
            <label className="text-xs text-slate-600 flex items-center gap-1" title="Opacidade (0-1)">Opac.<NumInput spin key={`${sb.id}-op`} value={sbst.opacity ?? 1} onChange={(v) => patchSelStyle({ opacity: v })} className="w-14 rounded border border-slate-300 px-1 py-1 text-sm" /></label>
            {sb.type === "TEXT" ? (
