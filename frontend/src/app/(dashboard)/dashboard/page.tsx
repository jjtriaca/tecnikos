...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ──── Agenda do Dia ──── */}
      {!loading && todayAgenda.length > 0 && (
        <div className="rounded-2xl border border-teal-200 bg-teal-50 shadow-sm">
          <div className="flex items-center justify-between border-b border-teal-200 px-6 py-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-teal-800">
                Agenda do Dia ({todayAgenda.length})
              </h3>
            </div>
            <Link
              href="/agenda"
              className="flex items-center gap-1 rounded-lg bg-teal-200 px-3 py-1.5 text-xs font-semibold text-teal-800 hover:bg-teal-300 transition-colors"
            >
              Ver agenda completa
              <IconArrowRight className="h-3 w-3" />
            </Link>
          ...
          <div className="divide-y divide-teal-200">
            {todayAgenda.slice(0, 5).map((os) => (
              <Link
                key={os.id}
                href={`/orders/${os.id}`}
                className="flex items-center justify-between px-6 py-3 hover:bg-teal-100/50 transition-colors"
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-teal-800">
                      {new Date(os.scheduledStartAt).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
                    </span>
                    <span className={`px-1.5 py-0.5 text-[10px] font-medium rounded ${STATUS_BADGE[os.status] || "bg-slate-100 text-slate-600"} border`}>
                      {STATUS_LABELS[os.status] || os.status}
                    </span>
                    {os.estimatedDurationMinutes && (
                      <span className="text-[10px] text-teal-600">{os.estimatedDurationMinutes}min</span>
                    )}
                  </div>
                  <p className="text-sm font-medium text-teal-900 truncate mt-0.5">{os.code} — {os.title}</p>
                  <div className="flex items-center gap-3 mt-0.5">
                    {os.assignedPartner && (
                      <span className="text-[11px] text-teal-600">{os.assignedPartner.name}</span>
                    )}
                    {os.clientPartner && (
                      <span className="text-[11px] text-teal-500">{os.clientPartner.name}</span>
                    )}
                    {os.city && (
                      <span className="text-[11px] text-teal-400">{os.city}</span>
                    )}
                  </div>
                </div>
              </Link>
            ...
            {todayAgenda.length > 5 && (
              <div className="px-6 py-2.5 text-center">
                <Link href="/agenda" className="text-xs font-medium text-teal-700 hover:text-teal-800">
                  + {todayAgenda.length - 5} mais agendamentos hoje
                </Link>
              </div>
            )}
          </div>
