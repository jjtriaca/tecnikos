...
import type { SupplierOnboardingConfig } from '@/types/stage-config';
...
interface Props {
  config: SupplierOnboardingConfig;
  onChange: (config: SupplierOnboardingConfig) => void;
}
  ...
  return (
    <label className="flex items-start gap-3 cursor-pointer group">
      <div className="relative mt-0.5">
        ...
        <div className="w-10 h-5 bg-slate-200 rounded-full peer peer-checked:bg-amber-600 peer-focus:ring-2 peer-focus:ring-amber-200 transition-colors after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-5 peer-checked:after:border-white" />
      </div>
...
function SubToggle({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <label className="inline-flex items-center gap-1.5 cursor-pointer">
      <input type="checkbox" checked={checked} onChange={e => onChange(e.target.checked)} className="rounded border-slate-300 text-amber-600 focus:ring-amber-200 h-3.5 w-3.5" />
...
function ConfigRow({ children, visible }: { children: React.ReactNode; visible: boolean }) {
  ...
  return <div className="ml-13 pl-3 border-l-2 border-amber-100 mt-2 space-y-2 animate-fadeIn">{children}</div>;
}

/* ── Variables ────────────────────────────────────────────── */

const SUPPLIER_WELCOME_VARIABLES = ['{nome}', '{empresa}', '{razao_social}', '{telefone}', '{email}', '{data}'];
const SUPPLIER_CONTRACT_VARIABLES = ['{nome}', '{empresa}', '{razao_social}', '{cnpj_empresa}', '{endereco_empresa}', '{documento}', '{email}', '{telefone}', '{data}'];
...
const DEFAULT_REPLY_MESSAGE = 'Perfeito {nome}! Seu cadastro como fornecedor esta confirmado. Em breve voce podera receber pedidos pela plataforma.';
const DEFAULT_DECLINE_MESSAGE = 'Atencao: o fornecedor {nome} ({email}) recusou o cadastro. Resposta: "{resposta}".';
...
export default function SupplierOnboardingSection({ config, onChange }: Props) {
  ...
  const update = (patch: Partial<SupplierOnboardingConfig>) =>
  ...
  return (
    <div className={`rounded-xl border-2 transition-all ${config.enabled ? 'border-amber-200 bg-white shadow-sm' : 'border-slate-200 bg-slate-50'}`}>
      {/* Header */}
      <div className="flex items-center justify-between p-4">
          ...
          {config.enabled && (
            <>
              {(config.sendContractLink || config.sendWelcomeMessage) && (
                <span className="ml-2 shrink-0 inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className={`text-xs font-medium ${config.enabled ? 'text-amber-600' : 'text-slate-400'}`}>
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            />
            <div className="w-10 h-5 bg-slate-200 rounded-full peer peer-checked:bg-amber-600 transition-colors after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-5 peer-checked:after:border-white" />
          </label>
        </div>
      </div>
      ...
      {!config.enabled && (
        <div className="px-4 pb-4">
          <p className="text-xs text-slate-400 italic">Desativado — ative para configurar contrato e boas-vindas ao cadastrar fornecedores</p>
        </div>
      )}
      ...
      {/* Expanded content */}
      {config.enabled && expanded && (
        <div className="px-4 pb-5 space-y-4 border-t border-amber-100 pt-4">

          {/* ── Contrato de Fornecimento ── */}
          <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-3 space-y-2">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 bg-slate-200 px-1.5 py-0.5 rounded">CONTRATO</span>
              <span className="text-xs text-slate-500">Contrato de fornecimento de materiais</span>
            </div>
            <Toggle
              checked={config.sendContractLink}
              onChange={(v) => update({ sendContractLink: v })}
              label="Enviar contrato de fornecimento"
              hint="Envia um link para o fornecedor visualizar e aceitar o contrato de fornecimento"
            />
            ...
            <ConfigRow visible={config.sendContractLink}>
              ...
              <label className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="text-xs rounded border border-slate-300 px-2 py-1 bg-white focus:border-amber-500 focus:ring-1 focus:ring-amber-200 outline-none"
                >
              ...
              <label className="flex flex-col gap-1">
                ...
                <input
                  ...
                  placeholder="Ex: Contrato de Fornecimento de Materiais"
                  className="text-sm rounded border border-slate-300 px-2 py-1.5 focus:border-amber-500 focus:ring-1 focus:ring-amber-200 outline-none"
                />
              </label>
              ...
              <div className="flex flex-col gap-1">
                ...
                <textarea
                  ...
                  placeholder="Digite o texto do contrato de fornecimento..."
                  rows={6}
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  ...
                  {SUPPLIER_CONTRACT_VARIABLES.map((v) => (
                    <button
                      ...
                      className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-500 hover:bg-amber-100 hover:text-amber-600 transition-colors"
                    >
                      {v}
                    </button>
                  ))}
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <SubToggle
                  ...
                  label="Bloquear fornecedor ate aceitar"
                />
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {expirationUnit !== 'indefinite' && (
                  <input
                    ...
                    className="text-xs rounded border border-slate-300 px-2 py-1 w-16 focus:border-amber-500 focus:ring-1 focus:ring-amber-200 outline-none"
                  />
                )}
                {expirationUnit === 'indefinite' && (
          ...
          <div className="rounded-lg border border-green-200 bg-green-50/30 p-3 space-y-2">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-slate-500">Boas-vindas ao fornecedor</span>
            </div>
            <Toggle
              checked={config.sendWelcomeMessage ?? false}
              onChange={(v) => update({ sendWelcomeMessage: v })}
              label="Enviar mensagem de boas-vindas"
              hint="Envia mensagem de boas-vindas ao novo fornecedor com opcao de confirmacao"
            />
            ...
            <ConfigRow visible={config.sendWelcomeMessage ?? false}>
              ...
              <div className="flex flex-col gap-1">
                ...
                <textarea
                  ...
                  placeholder="Digite a mensagem de boas-vindas para o fornecedor..."
                  rows={3}
              ...
              <div className="space-y-2">
                <SubToggle
                  ...
                  label="Aguardar confirmacao do fornecedor"
                />
                  ...
                  <div className="ml-5 space-y-3 animate-fadeIn">
                    ...
                    <p className="text-[10px] text-slate-400 italic">
                      {(config.welcomeConfirmVia || 'WHATSAPP') === 'WHATSAPP'
                        ? 'O fornecedor confirma respondendo qualquer mensagem no WhatsApp'
                        : 'O fornecedor confirma clicando em um link'}
                    </p>
                    ...
                    <div className="rounded border border-green-200 bg-green-50/50 p-2.5 space-y-2">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <textarea
                          ...
                          placeholder="Ex: Perfeito {nome}, seu cadastro como fornecedor esta confirmado!"
                          rows={2}
                          className="text-sm rounded border border-slate-300 px-2 py-1.5 focus:border-green-500 focus:ring-1 focus:ring-green-200 outline-none resize-y"
                        />
                    ...
                    <div className="rounded border border-red-200 bg-red-50/50 p-2.5 space-y-2">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                          ...
                          />
                          <span className="text-xs text-slate-600">Desativar cadastro do fornecedor</span>
                        </label>
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            type="checkbox"
