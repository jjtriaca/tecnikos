...
## ✅ PACOTE fontes + hierarquia de objetos (28/06, v1.14.74)
- [x] **Seletor de fonte corrigido:** lista única `FONTS` (module-level) usada no `<select>` E na detecção (`reflectSel` casa pelo PRIMEIRO token, igualdade — antes só 5 fontes hardcoded → as outras "voltavam pra Fonte"). Opção "Fonte" REMOVIDA (option vazia `disabled hidden`); **"Arial Black"** adicionada. Select reflete `selFmt.fontName || selectedBox.style.fontFamily` (caixa só selecionada também mostra a fonte).
- [x] **Hierarquia de objetos (camadas):** painel "🗂️ Objetos" na coluna esquerda (abaixo das páginas), só em página canvas. Lista `boxes` da região atual (page/header/footer) ordenado por z DESC. Clique = seleciona + aba Layout (localiza cards sobrepostos/escondidos). `Box.name` (novo campo) editável inline (✏️); sem nome → rótulo automático (`boxAutoLabel`: snippet do texto / "Imagem" / label do bloco / "Card"). ⬆⬇ reordena z (`moveLayer` troca z com vizinha); ✕ exclui. Funciona no cab/rodapé também.
- **Recuperação Capa (incidente G15):** backups pré-deploy em `/opt/tecnikos/backups/`; AuditLog (tabela `tenant_sls."AuditLog"`, cols before/after/createdAt) guarda snapshot de cada UPDATE → dá pra restaurar pageConfig.boxes do último snapshot bom. Capa SLS (id `44583d48-...`) tinha 16 boxes no último snapshot bom (28/06 23:30Z). Juliano optou por REFAZER a Capa (não restaurar). Página 2 (e5767c40, "Sobre Mim/Nossa História") intacta com 11 boxes.

## ✅ PACOTE validade + link-máscara + hierarquia reposicionada (28/06, v1.14.75)
- [x] **Hierarquia de objetos reposicionada:** painel compacto "🗂️ Objetos" no TOPO da coluna esquerda (acima das Páginas), **minimizado por padrão** (`objPanelOpen` state, toggle ▸/▾), texto 10px, lista com `max-h-44` scroll. Ordem de inserção (z asc; mais novo no fim). "Paginas" desceu (pt-1).
- [x] **Validade no orçamento (Condições gerais):** campo "Validade da proposta (dias)" inline em `BudgetSummaryBlock` (onUpdate validityDays) + botão **⭐ Salvar padrão**. Backend: `Company.systemConfig.pool.defaultValidityDays` (mesmo lugar de solarRules); endpoints `GET /pool-budgets/settings/proposal-defaults` + `PUT /pool-budgets/settings/validity`; create fallback `dto ?? template ?? tenantDefault ?? 30`. Página novo orçamento busca o default ao abrir.
